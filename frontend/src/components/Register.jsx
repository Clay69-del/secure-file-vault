...
function Register() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Check for the message from the backend indicating email verification
      if (response.data.message && response.data.message.includes("Please verify your email")) {
        setRegistrationSuccess(true); // Set success state
        toast.success(response.data.message); // Display toast message
      } else {
        // Fallback for unexpected success response (shouldn't happen with new backend)
        toast.success("Registration successful!");
        // Optionally, you could still log in or navigate if email verification is not strictly enforced
        // For now, we assume email verification is required.
      }

    } catch (error) {
      console.error("Registration error:", error);
      const errorMessage =
        error.response?.data?.error || // Use 'error' from backend if available
        ...
        <div className="max-w-md w-full space-y-8">
          ...
          {registrationSuccess ? (
            <div className="bg-green-50 border border-green-200 rounded-lg p-4 text-center space-y-4">
              ...
              <h3 className="text-xl font-semibold text-gray-800">Verification Email Sent!</h3>
              <p className="text-gray-600">
                Please check your inbox at <span className="font-medium text-purple-600">{formData.email}</span> to verify your account.
